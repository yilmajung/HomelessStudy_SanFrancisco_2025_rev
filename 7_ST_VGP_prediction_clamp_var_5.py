...
z95        = 1.96
z90        = 1.645
LOG_MIN    = -5.0   # lower clamp on log-rate
LOG_MAX    =  5.0   # upper clamp on log-rate

# Wrap your test_x in a DataLoader
test_loader = DataLoader(TensorDataset(test_x),
...
with torch.no_grad(), gpytorch.settings.fast_pred_var():
    for batch in tqdm(test_loader, desc="Computing clamped rate CIs"):
        x_batch = batch[0]                  # extract tensor
        post    = model(x_batch)            # GP posterior over f
        m       = post.mean                # (bsz,)
        s       = post.variance.sqrt()     # σ_f (bsz,)

        # Clamped log-intervals
        log_med  = m
        log_l95  = torch.clamp(m - z95 * s, min=LOG_MIN, max=LOG_MAX)
        log_u95  = torch.clamp(m + z95 * s, min=LOG_MIN, max=LOG_MAX)
        log_l90  = torch.clamp(m - z90 * s, min=LOG_MIN, max=LOG_MAX)
        log_u90  = torch.clamp(m + z90 * s, min=LOG_MIN, max=LOG_MAX)

        # Back to rate space
        rate_median  = torch.exp(log_med)
        rate_lower95 = torch.exp(log_l95)
        ...
        rate_lower95_list.append(rate_lower95.cpu().numpy())
        rate_upper95_list.append(rate_upper95.cpu().numpy())
        rate_lower90_list.append(rate_lower90.cpu().numpy())
        rate_upper90_list.append(rate_upper90.cpu().numpy())
        rate_std_list.append(rate_std.cpu().numpy())

# Flatten and attach to df_test
df_test = df_test.reset_index(drop=True)
df_test['rate_median']     = np.concatenate(rate_median_list)
df_test['rate_std']        = np.concatenate(rate_std_list)
df_test['rate_lower95']    = np.concatenate(rate_lower95_list)
df_test['rate_upper95']    = np.concatenate(rate_upper95_list)
df_test['rate_lower90']    = np.concatenate(rate_lower90_list)
df_test['rate_upper90']    = np.concatenate(rate_upper90_list)

# Quick check
print(df_test[['rate_median','rate_lower95','rate_upper95']].describe())
...
# Save results
df_test.to_csv('~/HomelessStudy_SanFrancisco_2025_rev_ISTServer/prediction_poisson_ip700_rev3_clamp5.csv', index=False)
print("Prediction complete. Results saved to 'prediction_poisson_ip700_rev3_clamp5.csv'.")
