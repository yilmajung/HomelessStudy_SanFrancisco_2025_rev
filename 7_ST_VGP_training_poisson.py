...
# Move model and Poisson likelihood to GPU
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = STVGPModel(inducing_points.to(device)).to(device)
likelihood = gpytorch.likelihoods.PoissonLikelihood().to(device) 
...
# Save the model
print("Saving model...")
torch.save(model.state_dict(), 'stvgp_model_poisson.pth')
joblib.dump(scaler, 'scaler.pkl')
torch.save(inducing_points, 'inducing_points_poisson.pt')
