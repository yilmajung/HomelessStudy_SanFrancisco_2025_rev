...
scaler = joblib.load('scaler_nb_500.pkl')
inducing_points = torch.load('inducing_points_nb_500.pt', map_location=device)
...
model.load_state_dict(torch.load('stvgp_model_nb_500.pth', map_location=device))
likelihood.load_state_dict(torch.load('stvgp_likelihood_nb_500.pth', map_location=device))
...
# Save results
df_test.to_csv('~/HomelessStudy_SanFrancisco_2025_rev_ISTServer/prediction_nb_t500r500.csv', index=False)
print("Prediction complete. Results saved to 'prediction_nb_t500r500.csv'.")
