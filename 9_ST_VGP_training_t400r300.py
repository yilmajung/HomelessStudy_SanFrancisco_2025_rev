...
joblib.dump(scaler, 'scaler_pois_velbo_t400r300.joblib')

# Compute log-mean for ConstantMean initialization
log_y_mean = np.log(y.mean() + 1e-3).astype(np.float32)
joblib.dump(log_y_mean, 'constant_mean_velbo_t400r300.pkl')
...
torch.save(likelihood.state_dict(), 'likelihood_pois_velbo_t400r300.pth')
torch.save(inducing_points, 'inducing_points_pois_velbo_t400r300.pt')
